...
public class BOJ_G4_14466_소가길을건너간이유6_fail {
    //백준 골드4
    //스터디 - 코테광탈
    // 시간초과
...
//            System.out.println(Arrays.toString(isSelected));
    ...
    static boolean bfs(Pos a, Pos b){
        ...
        while(!queue.isEmpty()){
            ...
            for(int d=0;d<4;d++){
                Pos next = new Pos(cur.num, cur.r+dr[d], cur.c+dc[d]);
                int nr = next.r;
                int nc = next.c;

                if(isIn(nr, nc)){
                    if(!isLoad(cur, next)){
                        queue.offer(new Pos(cur.num, nr, nc));
                    }
                }
